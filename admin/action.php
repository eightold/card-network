...
switch ($act){
    case 'sp_qiehuan':
        ...
        $zt = intval($_POST['zt']);
        $sql = "update if_goods set state = {$zt} where id = {$gid}";
        if($DB->query($sql)){
    ...
    case 'sp_plexe':
        ...
        if(intval($type) < 0){
            if(intval($type) == -2){
                //全部下架
                $sql = "update if_goods set state = 0 where id in ({$str})";
                if($DB->query($sql)){
                    exit('{"code":1,"msg":"操作成功！"}');
                }else{
                     exit('{"code":-1,"msg":"操作失败！"}');
            ...
            if(intval($type) == -3){
                //全部上架
                $sql = "update if_goods set state = 1 where id in ({$str})";
                if($DB->query($sql)){
                    exit('{"code":1,"msg":"操作成功！"}');
                }else{
                     exit('{"code":-1,"msg":"操作失败！"}');
            ...
            if(intval($type) == -4){
                //全部删除
                 //全部下架
                $sql = "delete from if_goods where id in ({$str})";
                if($DB->query($sql)){
                    exit('{"code":1,"msg":"操作成功！"}');
                }else{
                     exit('{"code":-1,"msg":"操作失败！"}');
            ...
            //移动分类
            $sql = "update if_goods set tpId = $type where id in ($str)";
            if($DB->query($sql)){
                    exit('{"code":1,"msg":"操作成功！"}');
            }else{
                     exit('{"code":-1,"msg":"操作失败！"}');
    ...
    case 'getsp':
        ...
        $sql = "select * from if_goods where tpId = ".$cid;
    ...
    case 'getkminfo':
        ...
        $sql1 = "select count(*) from if_km where gid = ".$tid;
        $sql2 = "select count(*) from if_km where stat = 0 and gid = ".$tid;
        $c1 = $DB->count($sql1);
         $c2 = $DB->count($sql2);
    ...
    case 'exekm':
        ...
        $str = daddslashes($_POST['str']);
        if($type == 1){
            $sql = "update if_km set stat = 1 where id in ($str)";
            $DB->query($sql);
             exit('{"code":1,"msg":"操作成功！"}');
        }elseif($type == 2){
            $sql = "update if_km set stat = 0 where id in ($str)";
            $DB->query($sql);
             exit('{"code":1,"msg":"操作成功！"}');
        }elseif($type == 3){
            $sql = "delete from if_km where id in ($str)";
            $DB->query($sql);
            exit('{"code":1,"msg":"操作成功！"}');
        }else{
    ...
    case 'getkmxxinfo':
        $id = intval($_POST['id']);
        $sql = "SELECT gName,km,benTime,endTime,out_trade_no,trade_no,rel,stat FROM `if_km` inner join if_goods on if_km.gid = if_goods.id where if_km.id = ".$id;
        $row = $DB->get_row($sql);
        if($row){
    ...
    case 'e_send_t':
        ...
        if($x->send($conf['mail_name'],$conf['mail_name'],$conf['mail_title'],"这是一封测试邮件！表示成功！[if]",$conf['title'])){
