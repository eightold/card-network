...
function stGName($gId,$DB){
    $sql = "select * from if_goods where id =".$gId;
    $rest = $DB->query($sql);
    $rowt = $DB->fetch($rest);
    return $rowt['gName'];
}
...
<?php }elseif($act == "query") {
    $type = $_POST['kind'];
    $txt = $_POST['value'];
   
    if($type == 1){
        $sql = "select * from if_km where km like '%{$txt}%'";
    }else if($type == 2){
        $sql = "select * from if_km where out_trade_no like '%{$txt}%' or trade_no like '%{$txt}%'";
    }else if($type == 3){
        $sql = "select * from if_km where rel like '%{$txt}%'";
    }
   // echo $sql;
    $rsss=$DB->query($sql);
   // echo $rsss;
